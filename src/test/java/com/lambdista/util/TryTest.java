...
public class TryTest {
        ...
        result.forEach(
                new Consumer<Integer>() {
                    ...
                    public void accept(Integer integer) {
                        assertEquals("i must be 42", (int) integer, 42);
                    }
                }
        );
    }
    ...
    public void testFlatMapAgainstASuccess() {
        ...
        Try<String> flatMappedResult = result.flatMap(
                new Function<Integer, Try<String>>() {
                    @Override
                    public Try<String> apply(final Integer i) {
                        return Try.apply(
                                new FailableSupplier<String>() {
                                    @Override
                                    public String get() throws Exception {
                                        return i + ", " + anotherSuccess();
                                    }
                                }
                        );
                    }
    ...
    public void testRecoverWithAgainstASuccess() {
        ...
        Try<Integer> recoveredResult = result.recoverWith(
                new Function<Exception, Try<Integer>>() {
                    @Override
                    public Try<Integer> apply(Exception exception) {
                        if (exception instanceof NumberFormatException) {
    ...
    public void testTransformAgainstASuccess() {
        ...
        Try<Integer> out = result.transform(
                new Function<Integer, Try<Integer>>() {
                    @Override
                    public Try<Integer> apply(Integer i) {
                        return new Try.Success<>(i + 42);
                    }
                },
                new Function<Exception, Try<Integer>>() {
                    @Override
